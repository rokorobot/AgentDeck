...
- **TypeScript main compile**:
  ...
  ```
- **Automated tests** (407 tests on `main` — a `node` project of 351 + a `renderer` jsdom project of 56; `npm audit` at 0):
  ```powershell
  npm test
  npm run build
  npm audit
...
| **W6-3 p3** | DepSlice / Decision Evidence Package candidate — characterization tests FIRST. Known wrinkle to pin: `signAndSaveDecisionEvidence` reloads `releaseCandidates` via `window.api.governance.loadData` and writes governance state on success (cross-domain write). | not started |
...
**To resume — W6-3 p3: DepSlice / Decision Evidence Package candidate.** Required order:
1. **FIRST** add DEP *characterization* tests that pin the current, unmoved behavior of the Decision Evidence Package state and actions in `workspaceStore.ts`.
2. **Known wrinkle to pin before moving anything:** `signAndSaveDecisionEvidence` reloads `releaseCandidates` via `window.api.governance.loadData(...)` and writes governance state (`set({ releaseCandidates: ... })`) on success — a cross-domain read+write that must be characterized and preserved, not silently dropped or re-timed, when this action moves.
3. **ONLY AFTER** those characterization tests are green should `DepSlice` extraction begin, following the same pattern as `doctorSlice.ts` / `provenanceSlice.ts`.
4. Not started. Do not move any DEP state/actions before the characterization tests pass.
