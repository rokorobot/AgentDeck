...
- **`EvaluationsView` decomposition** — break the largest component (1,383 lines, 7 sub-features) into per-tab components. **In progress as v1.0.9 / W6-1**, one tab per behavior-preserving PR into `src/components/evaluations/`: `BenchmarksTab` ✅ (PR #27), `RegressionTab` ✅ (PR #28), `ApprovalsTab` ✅ (PR #29, merge `1ae3455`), `FailuresTab` ✅ (PR #31, merge `39f71de`). Remaining: judges & definitions → gold standards → promotion history. Next up: W6-1 p5, the Promotion History tab (preferred — lowest-risk, read-only); Judges & Definitions is saved for its own heavier PR. Guarded by the W6-0 renderer harness (jsdom Vitest project). Optional shared `<Tabs>`/`<Modal>` primitive (W6-2) only if duplication proves real. `EvaluationsView.tsx` 1,383 → 1,125 → 1,064 → **877 lines** so far.
