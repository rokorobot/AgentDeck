...
- **`EvaluationsView` decomposition** — break the largest component (1,383 lines, 7 sub-features) into per-tab components. **In progress as v1.0.9 / W6-1**, one tab per behavior-preserving PR into `src/components/evaluations/`: `BenchmarksTab` ✅ (PR #27), `RegressionTab` ✅ (PR #28), `ApprovalsTab` ✅ (PR #29, merge `1ae3455`). Remaining: failures → judges → gold-standards → promotions. Next up: W6-1 p4, the Failures (Failure Library) tab. Guarded by the W6-0 renderer harness (jsdom Vitest project). Optional shared `<Tabs>`/`<Modal>` primitive (W6-2) only if duplication proves real. `EvaluationsView.tsx` 1,383 → 1,125 → **1,064 lines** so far.
