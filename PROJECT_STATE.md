...
**Snapshot:** 2026-07-10 · current version **v1.0.8** (Audit Remediation & Platform Upgrade) · branch `main` @ `e9aef5d`
**Repo:** https://github.com/rokorobot/AgentDeck.git

Since the v1.0.8 tag, an internal-hardening arc (v1.0.9, untagged) has landed on top of it without a version bump: collision-safe IDs (W1), command-safety unification (W2), `scratch/` retirement into real tests (W3), a shared JSON I/O helper (W4), the **now-complete** `electron/main.ts` decomposition (W5, one IPC domain per PR — 12 PRs, all merged; plus W5.1 final closure, PR #24), the **now-complete** renderer `EvaluationsView` tab extraction (W6-1, 7 PRs, all merged), the **now-merged** W6-3 p0 store-slicing plumbing (PR #39 — pure type/scaffolding relocation, no domain behavior moved), the **now-merged** W6-3 p1 DoctorSlice extraction (PR #41 — first domain slice, characterization-tests-first), the **now-merged** W6-3 p2 ProvenanceSlice extraction (PR #43 — second domain slice, same characterization-tests-first pattern), and the **now-merged** W6-3 p3 DepSlice extraction (PR #45, merge `e9aef5d` — third domain slice, same pattern). See "Next Milestones" below for the completed W5/W5.1/W6-1 record, the merged W6-3 p0 plumbing, the merged W6-3 p1 DoctorSlice, the merged W6-3 p2 ProvenanceSlice, the merged W6-3 p3 DepSlice, and the W6-3 p4 scope-decision next step, and [`docs/roadmap.md`](docs/roadmap.md) for the full forward-looking list.
...
- **TypeScript main compile**:
  ...
  ```
- **Automated tests** (416 tests on `main` — a `node` project of 351 + a `renderer` jsdom project of 65; `npm audit` at 0):
  ```powershell
  npm test
  npm run build
  npm audit
...
| **W6-3 p2** | ProvenanceSlice — `provenanceList` + `loadProvenance`/`recordProvenance`, `activeWorkspace`-only outbound coupling per the design gate. **Characterization tests FIRST**, then extraction. Second domain slice moved out of `workspaceStore.ts`. | ✅ merged (`3848e11`, PR #43) |
| **W6-3 p3** | DepSlice / Decision Evidence Package candidate — `decisionEvidenceList` + `loadDecisionEvidence`/`generateDecisionEvidence`/`signAndSaveDecisionEvidence`/`verifyDecisionEvidence`/`exportDecisionEvidenceJson`/`exportDecisionEvidenceMarkdown`. **Characterization tests FIRST**, then extraction. Third domain slice moved out of `workspaceStore.ts`. Known wrinkle pinned: `signAndSaveDecisionEvidence` reloads `releaseCandidates` via `window.api.governance.loadData` and writes governance state on success (cross-domain write) — preserved unchanged. | ✅ merged (`e9aef5d`, PR #45) |
| **W6-3 p4** | Scope decision between SnapshotsSlice and AgentsSlice — not yet decided; begin with an inventory gate. | not started |
...
**W6-3 p2 — MERGED (PR #43, merge `3848e11`).** Second domain slice extracted from `workspaceStore.ts`, following the same proven pattern as DoctorSlice. **Characterization-first proof:** 6 new Provenance tests (`workspaceStore.characterization.test.tsx`) were added and passed **green against the inline (unmoved) code first**, then the extraction happened, and the **same tests passed unchanged afterward**. The store characterization suite is now **17/17**. `src/store/workspaceStore.ts` went from **2,294 → 2,251 lines**; new **`src/store/slices/provenanceSlice.ts`** (75 lines) holds `provenanceList` state plus the `loadProvenance` and `recordProvenance` actions. `recordProvenance` is an **inbound** utility called from several other domains via `get().recordProvenance(...)` (saveActiveWorkspace, promoteToBaseline, and others) — those callers stay in their own domains in `workspaceStore.ts` and keep calling `get().recordProvenance(...)` through the shared closure, unchanged. **All 8 existing `get().recordProvenance(...)` call sites remain in `workspaceStore.ts`** — verified unchanged. `workspaceStore.ts` now spreads `...createProvenanceSlice(set, get, store)` into the store creator.

**ACCEPTED RESIDUAL (read this before touching `loadEvalsData`):** exactly **one** `window.api.provenance.loadAll(...)` call remains in `workspaceStore.ts`, inside `loadEvalsData()`'s atomic cross-domain hydration batch. This is an **intentional p2 acceptance exception, not unfinished extraction debt**. `loadEvalsData()` owns a single atomic hydration batch across several domains; re-routing this one read through `get().loadProvenance()` would split that batch into a separate `await`/`set()` and change render timing — a behavior change p2 explicitly avoided. Both provenance **action** IPC calls (`loadAll` inside `loadProvenance()`, and `recordMutation` inside `recordProvenance()`) live in `provenanceSlice.ts` — 2 `window.api.provenance.*` calls there, exactly 1 in `workspaceStore.ts`. Do not "finish" this in a future session without re-evaluating the atomic-hydration-batch tradeoff first.

`useWorkspaceStore = create<WorkspaceStore>((set, get, store) => …)` hook identity is unchanged. `WorkspaceStore extends ProvenanceSlice` was a type-only/additive change. **No** components / `electron/**` / `electron/preload.ts` / `package.json` / `package-lock.json` changes. Tests: **407/407** (node 351/351 + renderer 56/56). `slices/` now holds exactly `types.ts` + `doctorSlice.ts` + `provenanceSlice.ts` — no further domain slices exist yet. `doctorSlice.ts` is unchanged from p1 (not touched in the p2 diff).

**W6-3 p3 — MERGED (PR #45, merge `e9aef5d`).** Third domain slice extracted from `workspaceStore.ts`, following the same proven pattern as DoctorSlice/ProvenanceSlice. **Characterization-first proof:** 9 new DEP tests (`tests/renderer/workspaceStore.characterization.test.tsx`) were added and passed **green against the inline (unmoved) code first**, then the extraction happened, and the **same tests passed unchanged afterward**. The store characterization suite is now **26/26**. `src/store/workspaceStore.ts` went from **2,251 → 2,155 lines**; new **`src/store/slices/depSlice.ts`** (129 lines) holds `decisionEvidenceList` state plus the `loadDecisionEvidence`, `generateDecisionEvidence`, `signAndSaveDecisionEvidence`, `verifyDecisionEvidence`, `exportDecisionEvidenceJson`, and `exportDecisionEvidenceMarkdown` actions. **Cross-domain behavior preserved:** `signAndSaveDecisionEvidence()` still reloads `releaseCandidates` via `window.api.governance.loadData(...)` and writes it into the store (`set({ releaseCandidates: ... })`) on success — this read+write is now inside `depSlice.ts`, unchanged in behavior. `workspaceStore.ts` now spreads `...createDepSlice(set, get, store)` into the store creator.

**ACCEPTED RESIDUAL (read this before touching `loadEvalsData`):** exactly **one** `window.api.dep.loadAll(...)` call remains in `workspaceStore.ts`, inside `loadEvalsData()`'s atomic cross-domain hydration batch. This is an **intentional p3 acceptance exception, not unfinished extraction debt** — the same rationale as the p2 provenance residual below: `loadEvalsData()` owns a single atomic hydration batch across several domains, and re-routing this one read through `get().loadDecisionEvidence()` would split that batch into a separate `await`/`set()` and change render timing. All 6 DEP **action** IPC calls live in `depSlice.ts`, exactly 1 `window.api.dep.*` call remains in `workspaceStore.ts`.

`useWorkspaceStore = create<WorkspaceStore>((set, get, store) => …)` hook identity is unchanged. `WorkspaceStore extends DepSlice` was a type-only/additive change. **No** components / `electron/**` / `electron/preload.ts` / `package.json` / `package-lock.json` changes. Tests: **416/416** (node 351/351 + renderer 65/65). `slices/` now holds exactly `types.ts` + `doctorSlice.ts` + `provenanceSlice.ts` + `depSlice.ts` — no further domain slices exist yet. `doctorSlice.ts` and `provenanceSlice.ts` are unchanged from p1/p2 (not touched in the p3 diff).
...
**Store line-count trend:** 2,449 (pre-W6-3) → 2,339 (p0) → 2,294 (p1) → 2,251 (p2) → **2,155 (p3)**.

**To resume — W6-3 p4: scope decision between SnapshotsSlice and AgentsSlice.** Not started; no runtime work should begin until this scope decision is made. Required order:
1. **FIRST**, run an inventory gate over both candidate domains before committing to either — do not assume the pattern used for doctor/provenance/dep transfers cleanly.
2. **SnapshotsSlice risk:** broader payload fan-in than the domains sliced so far — `createSnapshot` reads from multiple other domains, and `restoreSnapshot` chains `createSnapshot` → `loadEvalsData` → `addTimelineEvent` → `recordProvenance`. A snapshot-hydration residual (analogous to the accepted p2/p3 `loadEvalsData` residuals) is likely and should be anticipated, not discovered mid-extraction.
3. **AgentsSlice risk:** may be the structurally cleaner next cut, but routes through core actions (`saveActiveWorkspace`, `createTerminal`, `killTerminal`) that are heavily cross-referenced elsewhere in the store.
4. The inventory gate should determine which of the two is the safer/cleaner p4 cut before any characterization tests or extraction begin.
5. Not started. Do not create `snapshotsSlice.ts`, `agentsSlice.ts`, or any other slice file before the scope decision is made and characterization tests pass.
